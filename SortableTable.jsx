...
const SortableTable = () => {
  ...
  return (
    <div className="bg-green-100 p-6 rounded-xl w-full mx-auto mt-6 shadow-lg">
      <h2 className="text-2xl font-extrabold text-gray-800 mb-4  uppercase tracking-wider">
        Details
      </h2>
      ...
      <table className="w-full bg-white rounded-xl overflow-hidden shadow-md">
        <thead className="bg-green-200 text-gray-700">
          <tr>
            <th className="p-3"></th> {/* No checkbox before Description */}
            {["description", "paymentBy", "amount", "customerId", "paymentDate"].map((key) => (
              <th key={key} className="p-3 text-left">
                <button
                  onClick={() => handleSort(key)}
                  className="bg-white px-3 py-1 rounded-lg shadow-md flex items-center space-x-2"
                >
            ...
            <th className="p-3 text-left font-semibold">Options</th>
          </tr>
        </thead>
        <tbody>
          {data.map((item) => (
